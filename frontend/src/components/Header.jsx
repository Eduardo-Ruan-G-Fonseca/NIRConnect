...
export default function Header() {
  ...
  const voltarAoInicio = (e) => {
    e.preventDefault();
    setOpen(false);
    navigate('/');               // volta para a home dentro do SPA
  ...
  return (
    <header className="bg-emerald-800 py-2 fixed top-0 w-full z-50 shadow-md">
      <nav className="max-w-7xl mx-auto flex items-center justify-between h-16 px-4 md:px-12">
        ...
        {/* Logo */}
        <Link to="/" onClick={voltarAoInicio} className="flex items-center ml-0 md:ml-4">
          <img src={logo} alt="NIR CONNECT" className="h-14 md:h-16 object-contain logo-glow" />
